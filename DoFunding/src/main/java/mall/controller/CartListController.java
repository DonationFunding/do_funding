...
public class CartListController {
	...
	public String doAction(HttpSession session) {
		...
		Set<Integer> keylist = orderlists.keySet(); // key(상품번호)
		int totalAmount = 0;
		for(Integer pnum : keylist) {
			Integer qty = orderlists.get(pnum);
			System.out.println(pnum+":" + qty); 
			ProductBean pb = productDao.getProduct(pnum);
			
			ShoppingInfo sinfo=new ShoppingInfo();
			sinfo.setPnum(pb.getNum());
			sinfo.setPname(pb.getName());
			sinfo.setQty(qty);
			sinfo.setPrice(pb.getPrice());
			sinfo.setAmount(qty*pb.getPrice());
			
			totalAmount += qty*pb.getPrice();
			lists.add(sinfo);
		}
		
		session.setAttribute("shopLists", lists);
