...
public class CartCalculateController {
	...
	private String getPage = "redirect:/list.mall";
	...
	public String doAction(HttpSession session ) {
		...
		// 1,2
		// 3,4
		Set<Integer> klists = orderlists.keySet();
		...
		//orderdetails의 oid에 넣기
		int maxOid = orderDao.getMaxOid(); 
		System.out.println("maxOid2:" + maxOid);
		
		// 상품 재고 수량 감소
		for(Integer pnum : klists) {
			Integer qty = orderlists.get(pnum);
			productDao.stockUpdate(pnum , qty );
			OrderDetailBean odBean=new OrderDetailBean();
			odBean.setOid(maxOid);
			odBean.setPnum(pnum);
			odBean.setQty(qty);
			int cnt=orderDetailDao.insertOrderDetail(odBean);
		}
		
		// 회원 mpoint 누적
		memberDao.mpointUpdate(loginInfo.getId(),100);
